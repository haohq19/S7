...
def training_step(
        ...
        loss_type: str = 'cross_entropy',  # New
):
    """
    ...
    :param batch: the data consisting of [inputs, targets, integration_timesteps, lengths]
    :param dropout_key: Random key for dropout
    :param distributed: If True, apply reduce operations like psum, pmean etc
    :param loss_type: Type of loss to use ('cross_entropy', 'one_hot_cross_entropy', or 'mse')
...
def get_optimizer(opt_config):
    ...
    proj_lrs = ["B_proj","C_proj","step_proj_kernel","step_proj_bias","x_proj"]
    ssm_fn = map_nested_fn(
        lambda k, _: "ssm"
        if k in ssm_lrs
        else "proj" if k in proj_lrs
        else "regular"
    )
        ...
        tx = optax.multi_transform(
            {
                ...
                "proj": optax.adamw(
                    learning_rate=learning_rate_fn(lr=learning_rate * opt_config.lr_factor),
                    b1=0.9, b2=0.999,
                    weight_decay=opt_config.proj_weight_decay),
            },
            ssm_fn,
        )
...
def init_model_state(rng_key, model, inputs, steps, lengths, opt_config, ):#batch_inputs):
    """
