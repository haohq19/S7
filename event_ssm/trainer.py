...
class TrainerModule:
    ...
    def __init__(
        ...
        if self.log_config.wandb:
            ...
            if 'perplexity' in self.log_config.summary_metric:
                wandb.define_metric(self.log_config.summary_metric, summary='min')
            else:
            ...
            if self.log_config.wandb:
                ...
                if self.world_size > 1:
                    if isinstance(self.train_state.opt_state, MultiStepsState):
                        lr = jax_utils.unreplicate(self.train_state.opt_state.inner_opt_state.inner_states['ssm'].inner_state.hyperparams['learning_rate'])
                    else:
                        lr = self.train_state.opt_state.inner_states['ssm'].inner_state.hyperparams['learning_rate']
                else:
                    if isinstance(self.train_state.opt_state, MultiStepsState):
    ...
    def load_model(self):
        ...
        if self.world_size > 1:
            state = jax_utils.unreplicate(self.train_state)
            abs_dir = os.path.abspath(os.path.join(self.log_dir, 'checkpoints'))
            #raw_restored = checkpoints.restore_checkpoint(ckpt_dir=os.path.join(self.log_dir, 'checkpoints'), target=state)
            raw_restored = checkpoints.restore_checkpoint(ckpt_dir=abs_dir, target=state)
            
            self.train_state = jax_utils.replicate(raw_restored)
            del state
        else:
            #print("full_dir = ",os.path.join(self.log_dir, 'checkpoints'))
            #print("log_dir = ",self.log_dir)
            #print("abs_dir = ",os.path.abspath(os.path.join(self.log_dir, 'checkpoints')))
            abs_dir = os.path.abspath(os.path.join(self.log_dir, 'checkpoints'))
            self.train_state = checkpoints.restore_checkpoint(ckpt_dir=abs_dir, target=self.train_state)
