...
class PathFinder(ImageResolutionSequenceDataset):
    ...
    def setup(self, stage=None):
        print("pathfinder setup cache dir",self.cache_dir)
        if self.data_dir is None:
            self.data_dir = (
        ...
        (self.dataset_train,self.dataset_val,self.dataset_test,) = \
        torch.utils.data.random_split(dataset,[train_len, val_len, test_len],
...
class AAN(SequenceDataset):
    ...
    def init_defaults(self):
        return {
            ...
            "append_bos": False,
            "append_eos": True,
            "n_workers": 40,  # For tokenizing only
        }
